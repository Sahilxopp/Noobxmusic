...
from OpVirMusic import YouTube, app
from OpVirMusic.core.userbot import assistants
from OpVirMusic.misc import SUDOERS, pymongodb
from OpVirMusic.plugins import ALL_MODULES
from OpVirMusic.utils.database import (get_global_tops,
                                       get_particulars, get_queries,
                                       get_served_chats,
                                       get_served_users, get_sudoers,
                                       get_top_chats, get_topp_users)
from OpVirMusic.utils.decorators.language import language, languageCB
from OpVirMusic.utils.inline.stats import (back_stats_buttons,
                                           back_stats_markup,
                                           get_stats_markup,
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"🔗[Telegram Files and Audios](https://t.me/incorrect_krishna) ** played {count} times**\n\n"
                else:
...
async def overall_stats(client, CallbackQuery, _):
    ...
    text = f"""**🥂⟩ Bᴏᴛ's sᴛᴀᴛs ᴀɴᴅ ɪɴғᴏ /~**
    
**Mᴏᴅᴜʟᴇs ~** `{mod}`
**Cʜᴀᴛs ~** `{2000 + served_chats}`
**Usᴇʀs ~** `{served_users}`
**Bʟᴏᴄᴋᴇᴅ ~** `{blocked}`
**Sᴜᴅᴏᴇʀs ~** `{sudoers}`
